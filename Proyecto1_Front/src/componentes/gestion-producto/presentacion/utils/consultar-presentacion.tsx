...
export default function ConsultarPresentaciones() {
  ...
  const handleAltaPresentacion = () => modal.abrirAlta();
  ...
  const handleBuscarDesdefiltro = (filtros: FiltrosPresentacionValues) => setFiltrosPresentacion(filtros);

  useEffect(() => {
    if (filtrosInicializados) handleBuscarPresentaciones();
  }, [paginaActual, filtrosInicializados]);

  useEffect(() => {
    if (filtrosInicializados) handleBuscarPresentaciones(true);
  }, [filtrosPresentacion]);
  ...
  const handleSuccess = async (mensajeAlerta: string) => {
    modal.cerrar();
    addAlert({ type: TipoAlerta.SUCCESS, title: TituloAlerta.SUCCESS, message: mensajeAlerta, autoClose: true });
