...
export function DatosCards({ presentacion, onEditar, onDelete }: Props) {
  ...
  return (
    ...
    >
      <div className="mb-2">
        ...
        <p className="text-sm font-medium text-gray-800 line-clamp-2">{presentacion.denominacion}</p>
        {eliminada && (
      ...
      {!eliminada && (
        <div className="flex justify-end gap-1 pt-2 border-t border-gray-100">
          ...
          <ActionButton variant="delete" onClick={() => onDelete(presentacion.id)} title="Eliminar">
            <Trash size={16} />
          </ActionButton>
        </div>
      )}
